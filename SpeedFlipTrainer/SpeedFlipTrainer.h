...
private:
	...
	// DEBUT
	// SpeedFlipTrainer.h - Ajoutez dans la partie privée
	shared_ptr<bool> showCarAxes = make_shared<bool>(true); // Option pour afficher/masquer les axes
	float axisLength = 150.0f; // Longueur des axes en unités de jeu

	// SpeedFlipTrainer.h - Ajoutez cette fonction dans la partie privée
	void RenderCarAxes(CanvasWrapper canvas);

	// Add this declaration in the private section of the SpeedFlipTrainer class
	struct Orientation; // Forward declaration of Orientation struct
